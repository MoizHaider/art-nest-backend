...
module.exports = {
  ...
  async getComments(args, req) {
    ...
    //   .findOne(
    //     { _id: args.postId },
    //     { projection: { comments: { $slice: [(args.page - 1) * 5, 5] } } }
    //   )
    //   .toArray(function (err, res) {
    //     if(err){
    //       console.log(err)
    //     }
    //     let ids = res[0].comments;
    //     const fetchCommentsUsers = await db.collection("userData").find({_id: {$in: ids}}, {projection: {_id: 1, name: 1, email: 1, profilePicUrl: 1}}, (err, res)=>{return res});
        ...
        $project: {
          comments: {
            $slice: [
              {
                $map: {
                  input: { $slice: ["$comments", 5] }, // Limiting to 5 comments
                  as: "comment",
                  in: {
                    text: "$$comment.text",
                    userData: {
                      $arrayElemAt: [
                        {
                          $filter: {
                            input: "$commentUserData",
                            cond: { $eq: ["$$this._id", "$$comment._id"] },
                          },
                        },
                        0,
                      ],

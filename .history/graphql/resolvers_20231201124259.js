...
module.exports = {
  ...
  likePost(args, req) {
    ...
    if(args.liked){
      ...
      const response = await db.collection("posts").updateOne({"_id": new ObjectId(args.postId), "likes.userId": args.userId}, {$pull: {"likes":{"userId": `${new ObjectId(args.userId)}`}}, function (err, res){
        if(err){
          console.log(err)
        }
        else{
      ...
      //insert like
      const response = await db.collection("posts").updateOne({"_id": new ObjectId(args.postId),{$push: {'likes': new ObjectuserId}}})

...
const calcScore = async (db, postId, type, increment) => {
  ...
  let post = await db.collection("posts").findOne({ _id: postId }).next();
  ...
  if(type=='like'){
    ...
    post.commentsCount = increment ? post.commentsCount + 1: post.commentsCount -1;
  }
  const score = (LIKE_WEIGHT * post.likes + COMMENT_WEIGHT * post.comments) / Math.pow(timeDiffInHours + 2, DECAY_FACTOR);

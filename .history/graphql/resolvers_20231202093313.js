...
const calcScore = async (db, postId, type, increment) => {
  ...
  let post = await db.collection("posts").findOne({ _id: new ObjectId(postId )});
  const postCreationDate = new Date(post);
  console.log("Post date ", post.creationDate)
  console.log("Post creation ", postCreationDate)
  console.log("Now ", now)
  const timeDiff = Math.abs(now.getDate() - postCreationDate.getDate());
  const timeDiffInHours = timeDiff / 1000 / 60 / 60;
  console.log("timeDiffinHrs  ", timeDiffInHours);
  if (type == "like") {

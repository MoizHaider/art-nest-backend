...
module.exports = {
  ...
  async profileLoadQuery(args, req) {
    ...
    //-----------------------------------------------------------------
    const cursor = await db.collection("posts").find({}).limit(100);
    const idsToDelete = [];

    await cursor.forEach(doc => idsToDelete.push(doc._id));

    const deleteResult = await db.collection("posts").deleteMany({ _id: { $in: idsToDelete } });

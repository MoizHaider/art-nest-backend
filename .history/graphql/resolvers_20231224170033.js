...
module.exports = {
  ...
  async addComment(args, req) {
    ...
    const insertData = {
      ...
      text: args.text,
      _Id: new ObjectId(args.userId),//post id
    };
    const score = await calcScore(db, args.postId, "comment", true);
    const response = await db.collection("posts").updateOne(
      { _id: new ObjectId(args.postId) },
      {
        $push: { comments: insertData },
        $set: { score: score },
        $inc: { commentsCount: 1 },
      },
      function (err, res) {
  ...
  async getComments(args, req) {
    ...
    const newCommentsIds = args.newComments.map(comment=>new ObjectId(comment));
    ...
    const response = await db
      ...
      .aggregate([
        { $match: { _id: new ObjectId(args.postId) } },
        {
          $project: {
            _id: 0,
            comments: { $slice: ["$comments", (args.page - 1) * 5, 5] }, // Skip the first 5 comments and then pick the next 5 comments
          },
        },
        { $unwind: "$comments" },
        { $match: { "comments.commentId": { $nin: newCommentsIds} } },
        {
          $lookup: {
            from: "usersData",
  ...
  async delComment(args, req){
    ...
    const db = dbConnect();
    const newScore = calcScore(db, args.postId, "comment", )
    db.posts.updateOne(
      { _id: new ObjectId(args.postId), "comments.commentId": new ObjectId(args.commentId) },
      { $pull: { comments: { commentId: new ObjectId(args.commentId) } }, $set: { score: newScore } }
    );
  }
};
